...
1. Compiler Compatibility: Only supports GCC/Clang (uses GNU extensions). | 编译器兼容性：仅支持 GCC/Clang（使用 GNU 扩展）。
2. Thread Safety: Not thread-safe; requires external synchronization for multi-threaded use. | 线程安全：非线程安全，多线程需外部同步。
3. Error Checking: Check return values for critical operations. | 错误检查：重要操作需检查返回值。
4. Performance Note: 
